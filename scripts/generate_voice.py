...
def trim_audio_precision(file_path):
    try:
        ...
        if len(audio) > 200: audio = audio[200:]
...
def generate_audio(text, output_base="temp_audio", target_voice=None):
    ...
    success, provider = False, "Unknown"

    sanitized_text = sanitize_for_tts(text)
    buffered_text = f", {sanitized_text}"

    print(f"🎙️ [VOICE] Requesting AI Director's choice: {target_voice}...")
    try:
        settings = config_manager.get_settings()
        kokoro_voices = settings.get("voice_actors", {}).get("kokoro", ['am_adam'])
        
        # Verify the AI picked a valid voice, otherwise fallback to random
        ...
        if audio_chunks:
            ...
            success, provider = True, f"Kokoro ({chosen_voice})"
    ...
    if not success:
        ...
        if groq_client.generate_audio(buffered_text, final_wav):
            success, provider = True, "Groq Orpheus API"

    if not success: return False, provider, 0.0

    trim_success, duration = trim_audio_precision(final_wav)
    if not trim_success: return False, provider, 0.0
    ...
    try:
        ...
        if not generate_fallback_srt(sanitized_text, duration, srt_path): return False, provider, 0.0
