...
def load_visual_preferences():
    ...
    modifiers = "3D cinematic animation style, 8k, vertical"
...
def animate_image_to_video(image_path, output_filename):
    print(f"🎞️ [VISUALS] Processing Animation for {output_filename}...")
    try:
        # Re-simulated FFmpeg filter to handle 9:16 portrait strictly
        cmd = [
        ...
        print(f"⚠️ [VISUALS] FFmpeg animation failed: {e}")
        return False
...
def generate_imagen_image(prompt, output_path):
    ...
    try:
        ...
        with open(output_path, 'wb') as f: f.write(response.generated_images[0].image_bytes)
        return True
    except: return False
...
def download_pexels_fallback(query, output_filename):
    ...
    if not api_key: return False
    print(f"📽️ [VISUALS] AI generation failed. Sourcing Pexels video for: {query}")
    try:
...
def fetch_background(base_query, output_filename="temp_background.mp4"):
    ...
    print(f"🖼️ [VISUALS] Generating AI Background: {base_query}")
    if generate_imagen_image(full_prompt, temp_img) or generate_pollinations_image(full_prompt, temp_img):
        if animate_image_to_video(temp_img, output_filename):
            if os.path.exists(temp_img): os.remove(temp_img)
            return True

    # Step 2: Ultimate Failsafe - Pexels Real Footage
