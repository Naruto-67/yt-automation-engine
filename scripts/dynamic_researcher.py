...
def get_deep_channel_context(youtube):
    ...
    try:
        ...
        video_data = [{"title": i["snippet"]["title"], "views": int(i["statistics"].get("viewCount", 0)), "likes": int(i["statistics"].get("likeCount", 0)), "id": i["id"]} for i in stats_response.get("items", [])]
        ...
        try:
            ...
            for c in comments.get("items", []): fan_suggestions.append(c["snippet"]["topLevelComment"]["snippet"]["textOriginal"])
        except: pass
        
        context = "📊 CHANNEL PERFORMANCE REPORT (PAST 7 DAYS):\nTOP PERFORMING VIDEOS:\n"
        for v in top_vids: context += f"- Title: '{v['title']}' | Views: {v['views']} | Likes: {v['likes']}\n"
        if fan_suggestions:
            context += "\n💬 RECENT FAN COMMENTS / SUGGESTIONS:\n"
            for s in fan_suggestions: context += f"- \"{s}\"\n"
        return context
    except: return "Failed to fetch stats. Generate broadly appealing viral niches."
...
def run_dynamic_research():
    ...
    1. THE BASELINE: At least 2 "Short Story" topics and 2 "Facts" topics.
    2. THE EXPLOIT: 12 topics MUST double-down on whatever is currently working best in the channel data below.
    3. THE EXPLORE (WILDCARDS): 5 topics MUST be completely NEW, highly viral internet niches that are perfect for AI image generation (e.g., 'Analog Horror', 'Liminal Spaces'). 
    ...
    try:
        ...
        if match:
            ...
            # 🚨 FIX: PRESERVE THE VAULT! Load old memory, keep ONLY vaulted videos, merge with new topics.
            existing_matrix = []
            ...
            vaulted_items = [i for i in existing_matrix if i.get("processed", False) and not i.get("published", False)]
            random.shuffle(new_matrix)
            final_matrix = vaulted_items + new_matrix
            
            with open(matrix_path, "w", encoding="utf-8") as f:
                json.dump(final_matrix, f, indent=4)
                
            print(f"✅ [RESEARCHER] Matrix updated. Vault preserved ({len(vaulted_items)} items) + 21 new topics.")
