...
def render_video(image_paths, audio_path, output_path, scene_weights=None, watermark_text="GhostEngine", style_name="default"):
    ...
    # 🚨 WATERMARK: Clean, 35% opacity, bottom center.
    safe_font, safe_ass = download_cinematic_font().replace('\\', '/').replace(':', r'\:'), ass_path.replace('\\', '/').replace(':', r'\:')
    watermark_filter = f",drawtext=fontfile='{safe_font}':text='{watermark_text}':fontcolor=white@0.35:fontsize=50:x=(w-text_w)/2:y=h-200"
