...
# 2. Azure AI Inference Endpoint check (New endpoint for GitHub/Azure models)
...
for model in candidates:
    try:
        ...
        res = urllib.request.urlopen(req)
        print(f"   ✅ Model [{model}]: PASS (HTTP 200)")
...
# 3. Legacy GitHub Models Endpoint check
gh_endpoint = "https://models.github.ai/inference/chat/completions"
print("\n🔬 Testing Legacy GitHub Models Endpoint (https://models.github.ai/inference/chat/completions):")
for model in candidates:
    try:
        payload = json.dumps({"model": model, "messages": [{"role": "user", "content": "ping"}], "max_tokens": 2}).encode('utf-8')
        req = urllib.request.Request(
